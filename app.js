...
const App = (() => {
  ...
  const drawHeader = () => {
    ...
    const updateTime = () => {
      const time = new Date();
      date.setContent(
        `${zeroPad(time.getHours())}:${zeroPad(time.getMinutes())}:${zeroPad(
          time.getSeconds()
        )} `
      );
      screen.render();
  ...
  const drawChart = chartKey => {
    ...
    for (const pos in charts[chartKey].values) {
      if (graphScale >= 1 || (graphScale < 1 && pos % (1 / graphScale) === 0)) {
        const p =
          parseInt(pos, 10) + (chart.width - charts[chartKey].values.length);
        ...
        for (
          let y = computeValue(charts[chartKey].values[pos - 1]);
          y < chart.height;
          y++
        ) {
    ...
    textOutput[0] = `${textOutput[0].slice(
      0,
      textOutput[0].length - 4
    )}{white-fg}${percent.slice(-3)}%{/white-fg}`;
  ...
  const drawTable = chartKey => {
    ...
    for (const headerColumn in columns) {
      ...
      titleOutput +=
        ...
        stringRepeat(
          " ",
          columnLengths[columns[headerColumn]] - colText.length
        );
    }
    titleOutput += "{/bold}" + "\n";
    ...
    for (const row in chart.plugin.currentValue) {
      ...
      for (const bodyColumn in columns) {
        ...
        rowText += (
          colText +
          stringRepeat(" ", columnLengths[columns[bodyColumn]] - colText.length)
    ...
    init() {
      ...
      screen.on("keypress", (ch, key) => {
        ...
        if (lastKey === "d" && key.name === "d") {
          let selectedProcess = processListSelection.getItem(
            processListSelection.selected
          ).content;
      ...
      const setupCharts = () => {
        ...
        const plugins = ["cpu", "memory", "process", "temperature"];
        ...
        for (const plugin in plugins) {
          ...
          let values;
          if (
            typeof charts[plugin] !== "undefined" &&
            typeof charts[plugin].values !== "undefined"
          ) {
      ...
      intervals.push(
        setInterval(charts[3].plugin.poll, charts[3].plugin.interval)
      );
    }
  };
})();
